...
BEGIN
    ...
    /* now calculate win/total and write */
    /* handle player A */
	SELECT games_total INTO @playerA_total FROM players p WHERE p.id = @playerA_id;
	SELECT games_won INTO @playerA_won FROM players p WHERE p.id = @playerA_id;
    UPDATE players p SET p.games_win_lost = ( games_won / games_total ) * 100 WHERE p.id = @playerA_id;
    
    /* handle player B */
	SELECT games_total INTO @playerB_total FROM players p WHERE p.id = @playerB_id;
	SELECT games_won INTO @playerB_won FROM players p WHERE p.id = @playerB_id;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteGame`(IN gameId_p INT(11))
BEGIN
...
-- Dump completed on 2018-09-13 18:55:48
